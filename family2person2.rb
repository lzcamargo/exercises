module FamilyPerson
	...
	state do # model specified in two collections 
		table :family, [:obj_name, :att_name, :att_instance]
		table :member, [:obj_name, :att_name, :att_instance, :att_anc_inst]
	end
	state do
		scratch :member_gender, [:fg_name, :fg_role, :fg_fname] # collection used in helper
		table :person, [:obj_name, :att_name, :att_instance] # target collection
	end	
	
	bloom :heper_f2p do #helper for selecting the member genders
		member_gender <= (person_mm * family_role_mm).pairs do |p, f| 
			if (p.class_name == "Male" and (f.role_name == "father" or f.role_name == "sons"))
			  [p.class_name, f.role_name, p.att_name]
			elsif (p.class_name == "Female" and (f.role_name == "mother" or f.role_name == "daugthers"))	
	      [p.class_name, f.role_name, p.att_name]
			end
		end
	end
